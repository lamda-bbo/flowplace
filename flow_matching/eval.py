...
import time
import wandb
from benchmark_utils import iccad2015_utils, ispd2005_utils
from benchmark_utils import openroad_utils
...
def main(cfg):
    ...
    for i in range(cfg.num_output_samples):
        x, cond = val_set[i]
        if cfg.output_placement:
            metrics, metrics_special, image, image_legalized, sample = common.eval_functions.save_outputs_with_solution(
                x, 
                cond, 
                model, 
        ...
        placement_dir = os.path.join(log_dir, "placements")
        os.makedirs(placement_dir, exist_ok=True)
        if cfg.output_placement:
            if 'iccad2015' in cfg.task:
                print(f"Converting to OpenROAD format for benchmark {cond.benchmark_name} to {placement_dir}")
                iccad2015_utils.convert_to_iccad15(sample, benchmark_name=cond.benchmark_name, 
                    output_path= placement_dir)
            elif 'ispd2005' in cfg.task:
                print(f"Converting to OpenROAD format for benchmark {cond.benchmark_name} to {placement_dir}")
                ispd2005_utils.convert_to_ispd2005(sample, cond,  benchmark_name=cond.benchmark_name, output_path= placement_dir)
            elif 'openroad' in cfg.task:
                print(f"Converting to OpenROAD format for benchmark {cond.benchmark_name} to {placement_dir}")
                openroad_utils.convert_to_openroad(sample,  cond.benchmark_name, output_dir= placement_dir)
