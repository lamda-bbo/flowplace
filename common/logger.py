...
class Logger:
  ...
  def add(self, mapping, prefix=None):
    ...
    for name, value in dict(mapping).items():
      ...
      if isinstance(value, plt.Figure) or isinstance(value, wandb.Html):
        pass
      elif isinstance(value, str) or isinstance(value, dict): # and isinstance(next(iter(value.values())), str)):
        pass
      else:
...
class WandBOutput:
  ...
  def __init__(self, name=None, config=None, pattern=r'.*'):
    self._pattern = re.compile(pattern)
    # Safely extract project/entity from optional OmegaConf config
    project = None
    entity = None
    if config and getattr(config, 'logger', None):
      project = getattr(config.logger, 'wandb_project', None)
      entity = getattr(config.logger, 'wandb_entity', None)
    # Convert OmegaConf to a plain container and ensure dict keys are strings
    cfg = OmegaConf.to_container(config, resolve=True) if config else None
    if isinstance(cfg, dict):
      cfg = {str(k): v for k, v in cfg.items()}
    # Use 'name' for the run name and do not misuse the 'resume' parameter
    wandb.init(
        project=project,
        name=name,
    ...
    for step, name, value in summaries:
      ...
      if isinstance(value, plt.Figure) or isinstance(value, wandb.Html):
        ...
        if not name in tables:
          tables[name] = wandb.Table(columns = list(value.keys()))
        tables[name].add_data(*list(value.values()))
        bystep[step][name] = tables[name]
      elif len(value.shape) == 0:
        bystep[step][name] = float(value)
      elif len(value.shape) == 1:
