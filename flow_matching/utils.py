...
def validate(x_val, model, cond=None):
    ...
    if cond is None:
        loss, model_metrics = model.loss(x_val, t, split = "val")
    else:
        loss, model_metrics = model.loss(x_val, cond, t, split = "val")
    logs = {
        "loss": loss.cpu().item()
    }
...
def generate_report(num_samples, dataloader, model, logger, policy = "iterative", intermediate_every = 200):
    ...
    for _ in range(num_samples):
        x_eval, cond_eval, _ = dataloader.get_batch("val")
        x_eval = x_eval[:1]
...
def load_finetune_data_with_config(dataset_name, train_data_limit = None, override_placement_path = None, placement_format = "output*.pickle"):
    ...
    if os.path.exists(dataset_path):
        ...
        if train_data_limit is None or train_data_limit == "none":
            train_data_limit = config.train_samples
        train_idx = idx_list[:train_data_limit]
        val_idx = idx_list[train_data_limit:]
        ...
        for train_i in train_idx:
            ...
            if config.get("preprocessed", False):
                pass
                print(f"Data for graph {train_i} is already preprocessed.")
            else:
                print(f"Preprocessing data for graph {train_i}.")
        ...
        for val_i in val_idx:
            ...
            if config.get("preprocessed", False):
                pass
                print(f"Data for graph {val_i} is already preprocessed.")
            else:
                print(f"Preprocessing data for graph {val_i}.")
                x, cond = preprocess_graph(x, cond, chip_size, scale, chip_offset=chip_offset)
            cond.file_idx = val_i
            if dataset_name == "ispd2005":
...
def load_graph_data_with_config(dataset_name, train_data_limit = None, val_data_limit = None, override_placement_path = None, placement_format = "output*.pickle"):
    ...
    if os.path.exists(dataset_path):
        ...
        if val_data_limit is None or val_data_limit == "none":
            val_data_limit = VAL_SIZE
        assert train_data_limit <= TRAIN_SIZE and val_data_limit <= VAL_SIZE, f"data limits invalid: train_data_limit {train_data_limit} <= TRAIN_SIZE {TRAIN_SIZE} and val_data_limit {val_data_limit} <= VAL_SIZE {VAL_SIZE}"
...
def preprocess_synthetic_graph(x, cond, needs_centering=False):
    ...
    # print(f" shapes of x, cond.x, cond.edge_index, cond.edge_attr: {x.shape}, {cond.x.shape}, {cond.edge_index.shape}, {cond.edge_attr.shape}")
    # exit()
    return x, cond
...
class GraphDataLoader:
    ...
    def get_batch(self, split):
        ...
        if self.is_shuffle[split]:
            idx = torch.randint(0, len(dataset), [1]).item()  # .item() to int
        else:
    ...
    def get_display_batch(self, display_batch_size, split="val"):
        ...
        if (self._display_x.get(split, None) is None) or (self._display_y.get(split, None) is None):
            x, y, idx = self.get_batch(split)  # 添加 idx
...
def load_and_parse_graph(path):
    ...
    for key in ["is_macros", "is_ports"]:
        if key in graph and len(graph[key].shape) > 1:
            graph[key] = torch.flatten(graph[key]).bool() 
    return graph
