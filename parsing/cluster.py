...
def main(cfg):
    ...
    print(f"==== Running clustering for benchmark {benchmark_name} ====")
    ...
    if cfg.use_placed_def:
        ...
        lef_pattern = r"^(\S+)_placed.def"
    else:
        def_files = sorted(list(pathlib.Path(cfg.benchmark_dir).rglob("*.def")))
        lef_pattern = r"^(\S+).def"
    print(f"Found {len(def_files)} DEF files to cluster.")
    print(f"def files: {def_files[:5]} ... {def_files[-5:]}")
